...
public class Sound
{
    ...
    static float before_bgm = -1.0f, before_se = -1.0f;
    ...
    class _Data
    {
        ...
        public _Data(string key, string res, eType type)
        {
            Key = key;
            if (type == eType.Bgm) { ResName = "Sounds/Bgm/" + res; }
            if (type == eType.Se) { ResName = "Sounds/Se/" + res; }
            // AudioClipの取得
            Clip = Resources.Load(ResName) as AudioClip;
        }
    }
    ...
    void _LoadBgm(string key, string resName)
    {
        ...
        _poolBgm.Add(key, new _Data(key, resName, eType.Bgm));
    }
    void _LoadSe(string key, string resName)
    {
        ...
        _poolSe.Add(key, new _Data(key, resName, eType.Se));
    }
    ...
    /*自作関数*/
    //ボリューム変更（BGM）
    public static void ChangeVolumeBgm(float vol)
    {
        //音量が同じ場合処理しない
        if (before_bgm == vol) return;
        
        Debug.Log("Change BGM Volume" + vol);
         
        var source = GetInstance()._GetAudioSource(eType.Bgm);
        source.volume = vol;

        before_bgm = vol;
    }

    public static void ChangeVolumeSe(float vol)
    {
        //音量が同じ場合処理しない
        if (before_se == vol) return;

        Debug.Log("Change SE Volume" + vol);

        var source = GetInstance()._GetAudioSource(eType.Se);    
        source.volume = vol;

        for (int i = 0; i < SE_CHANNEL;i++)
        {
            source = GetInstance()._GetAudioSource(eType.Se,i);
            source.volume = vol;
        }
